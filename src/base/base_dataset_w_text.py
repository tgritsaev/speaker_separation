...
class BaseDatasetWText(Dataset):
    def __init__(
        self,
        index,
    ...
    @staticmethod
    def _filter_records_from_dataset(index: list, max_audio_length, max_text_length, limit) -> list:
        ...
        if max_text_length is not None:
            exceeds_text_length = (
                np.array([len(BaseTextEncoder.normalize_text(el["text"])) for el in index]) >= max_text_length
            )
    ...
    def _assert_index_is_valid(index):
        for entry in index:
            assert "audio_len" in entry, (
                "Each dataset item should include field 'audio_len'" " - duration of audio (in seconds)."
            )
            assert "path" in entry, "Each dataset item should include field 'path'" " - path to audio file."
            assert "text" in entry, (
                "Each dataset item should include field 'text'" " - text transcription of the audio."
            )
